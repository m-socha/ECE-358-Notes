...
\begin{document}
  ...
  \section{Transport Layer}
      ...
      \subsubsection{Causes of Congestion Control}
        ...
        \begin{enumerate}
          ...
          \item \textbf{Multiple senders, routers with finite buffers, multihop paths.} Network congestion can cause delays or packet loss between different hosts that happen to be
            communicating through the same infrastructure (i.e. same router). More routers involved in delivering packets opens the possibility for more wasted effort, since
    ...
    \subsection{TCP Congestion Control}
      ...
      When a TCP connection is first established, the initial cwnd is set very low (1 minimum segment size (MSS)). However, on each successful ACK, this segment size doubles. Once some
      ...
      this ``fast recovery'') or is set back to 1 MSS (the older TCP Tahoe). If a timeout occurs, cwnd is set back to 1 MSS in both TCP Tahoe and Reno.
